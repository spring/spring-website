...
class WP_Multi_Importer_Widget extends WP_Widget {
	...
	(array) $catArray = $instance['category'];
		...
		if (((!in_array(0, $catArray ) && in_array(intval($rssmi_cat), $catArray ))) || in_array(0, $catArray )) {
			...
			$myfeeds[] = array("FeedName"=>$rssmi_title,"FeedURL"=>$rssmi_url,"FeedCatID"=>$rssmi_cat); //with Feed Category ID
		...
		if (!isset($myfeeds) || empty($myfeeds)) return;
...
echo '	<div class="news-contents">';
		...
		foreach($myarray as $items) {
			...
			echo '<a '.$openWindow.' href="'.$items["mylink"].'" class="news_title">'.$items["mytitle"].'</a><br />';
